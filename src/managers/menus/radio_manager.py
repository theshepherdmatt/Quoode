...
class RadioManager(BaseManager):
    ...
    def update_song_info(self, state):
        ...
        self.logger.info("RadioManager: Updating playback metrics display.")
        ...
        if self.mode_manager.original_screen:
            ...
            self.logger.debug("RadioManager: Forwarded playback metrics to OriginalScreen.")
        else:
            self.logger.error("RadioManager: OriginalScreen is not set in ModeManager.")