...
class SpotifyManager(BaseManager):
    ...
    def update_song_info(self, state):
        ...
        if self.mode_manager.original_screen:
            ...
            self.logger.debug("SpotifyManager: Forwarded playback metrics to OriginalScreen.")
        else:
            self.logger.error("SpotifyManager: OriginalScreen is not set in ModeManager.")
