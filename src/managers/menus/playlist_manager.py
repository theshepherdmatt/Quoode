...
class PlaylistManager(BaseManager):
    ...
    def update_song_info(self, state):
        ...
        if self.mode_manager.original_screen:
            ...
            self.logger.debug("PlaylistManager: Forwarded playback metrics to OriginalScreen.")
        else:
            self.logger.error("PlaylistManager: OriginalScreen is not set in ModeManager.")